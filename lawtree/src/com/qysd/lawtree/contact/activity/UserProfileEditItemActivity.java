...
public class UserProfileEditItemActivity extends UI implements View.OnClickListener {
    ...
    // gender activity_account_and_security
    ...
    // birth activity_account_and_security
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (key == UserConstant.KEY_NICKNAME || key == UserConstant.KEY_PHONE || key == UserConstant.KEY_EMAIL
                ...
                || key == UserConstant.KEY_IDCARD || key == UserConstant.KEY_FIRM) {
    ...
    private void setTitles() {
        switch (key) {
            ...
            case UserConstant.KEY_EMAIL:
                ...
                break;
            case UserConstant.KEY_IDCARD:
                setTitle(R.string.idcard);
                tv_title.setText("身份证号");
                break;
            case UserConstant.KEY_FIRM:
                setTitle(R.string.firm);
                break;
            case UserConstant.KEY_SIGNATURE:
                setTitle(R.string.signature);
                tv_title.setText(R.string.signature);
                break;
            ...
            case UserConstant.KEY_BIRTH:
                ...
                tv_title.setText(R.string.birthday);
                break;
    ...
    private void initActionbar() {
        findView(R.id.linear_comment_back).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        findView(R.id.tv_save).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (key == UserConstant.KEY_BIRTH) {
                    EventBus.getDefault().post(new PersonalInfoBean(UserConstant.KEY_BIRTH, birthText.getText().toString()));
                    update(birthText.getText().toString());
                } else if (key == UserConstant.KEY_GENDER) {
                    update(Integer.valueOf(gender));
                }else if (key == UserConstant.KEY_NICKNAME) {
                    EventBus.getDefault().post(new PersonalInfoBean(UserConstant.KEY_NICKNAME, editText.getText().toString().trim()));
                    finish();
                } else if (key == UserConstant.KEY_IDCARD) {
                    EventBus.getDefault().post(new PersonalInfoBean(UserConstant.KEY_IDCARD, editText.getText().toString().trim()));
                    finish();
                }else if (key == UserConstant.KEY_PHONE) {
                    EventBus.getDefault().post(new PersonalInfoBean(UserConstant.KEY_PHONE, editText.getText().toString().trim()));
                    finish();
                }else if (key == UserConstant.KEY_SIGNATURE) {
                    EventBus.getDefault().post(new PersonalInfoBean(UserConstant.KEY_SIGNATURE, editText.getText().toString().trim()));
                    finish();
                } else {
