...
public class AddQunPersonMqyActivity extends BaseActivity {
    ...
    protected void initView() {
        ...
        selectedPersonBeanList = (List<SelectPersonBean.Status>) mqy.getSerializable("mqylist");
...
//        lv_content.addHeaderView(inflate);
    ...
    private void loadData() {
        Log.e("account lzq", NimUIKit.getAccount());
        OkHttpUtils.get().url(Constants.baseUrl + "company/selectCompanyUserInfo")
                ...
                .execute(new UserCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        adapter = new MqySelectPingyinAdapter<SelectPersonBean.Status>(lv_content, selectPersonBeanList, R.layout.item_select_add_lianxiren) {
                                ...
                                class DataViewHolder extends ViewHolder implements View.OnClickListener, View.OnLongClickListener {
                                    ...
                                    public void onClick(View v) {
                                        ...
                                        if (newIsSelected.get(getItem().getUserId()) == 1) {
                                            ...
                                            EventBus.getDefault().post(new AddQunLiaoEventBusBean("mqyadd", getItem().getUserId(), 2));
                                        } else if (newIsSelected.get(getItem().getUserId()) == 2) {
                                            iv_cb.setImageResource(R.drawable.ic_check_normal);
                                            newIsSelected.put(getItem().getUserId(), 1);
                                            ...
                                            Log.e("userid--", getItem().getUserId());
                                            EventBus.getDefault().post(new AddQunLiaoEventBusBean("mqyadd", getItem().getUserId(), 1));
                                        }
                                        Log.e("hhh", newselectedPersonBeanList.size() + "");
                                        //tv_select_all_num.setText("已选择：" + (count + newselectedPersonBeanList.size() + mqyList.size() + wbqyList.size()) + "人");
                                        //tv_option_num.setText("确定(" + (count + newselectedPersonBeanList.size() + mqyList.size() + wbqyList.size()) + "/200)");
                                    }
    ...
    protected void onDestroy() {
        EventBus.getDefault().post(new AddQunLiaoEventBusBean("mqy", newselectedPersonBeanList));
        Log.e("mqy add", newselectedPersonBeanList.size() + "");
        super.onDestroy();
    }
}
